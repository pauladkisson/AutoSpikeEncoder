...
class SpikePreProcessor:
    ...
    def __call__(self, raw_data, vis=None):
        ...
        (
            ...
            max_spike_voltages,
            max_lfp_voltages,
        ) = self.normalize(aligned_spikes, lfp)
        if vis:
            self.visualize(
                raw_data,
                bp_data,
    ...
    def threshold(self, data):
        ...
        for channel in range(self.num_channels):
            peak_indices, _ = find_peaks(
                (data[:, channel]), thresh[channel], width=self.spike_width
            )
    ...
    def align(self, data, spike_indices):
        ...
        for channel in range(self.num_channels):
            ...
            for i, spike_idx in enumerate(channel_spike_indices):
                # obtain raw spike
                spike_window = (
                    spike_idx - self.align_radius,
                    spike_idx + self.align_radius + 1,
                )
                if spike_window[0] < 0 or spike_window[1] > len(
                    t
                ):  # spike occurs within 1ms of start or end
                    continue
                ...
                new_spike_window = (
                    new_spike_idx - self.align_radius,
                    new_spike_idx + self.align_radius + 1,
                )
                if new_spike_window[0] < 0 or new_spike_window[1] > len(
                    t
                ):  # aligned spike occurs within 1ms of start or end
                    continue
                ...
                # record
                if np.isin(
                    spike_time, np.array(channel_spike_times)
                ):  # spike has already been detected
                    continue
                else:
    ...
    def visualize(
        ...
        for channel in range(self.num_channels):
            if np.all(raw_data[:, channel] == 0):  # dead channel
                ...
                continue
            normed_raw_data = raw_data[:, channel] / np.max(
                np.abs(raw_data[:, channel])
            )
            ...
            assert spike_train.sum() == spike_times[channel].size, (
                "Incorrect spike train construction: spike train should have %s spikes but has %s spikes instead"
                % (spike_times[channel].size, spike_train.sum())
            )
