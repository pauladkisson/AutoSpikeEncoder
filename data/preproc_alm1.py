...
for session_num, url in enumerate(urls):
    ...
    for trial_num, trial_data in enumerate(raw_data):
        ...
        trial_pathname = os.path.join(
            alm_dir, f"session_{session_num + start_session}", f"trial_{trial_num}"
        )
        ...
        np.save(
            os.path.join(trial_pathname, "max_spike_voltages.npy"),
            max_spike_voltages,
            allow_pickle=True,
        )
        np.save(
            os.path.join(trial_pathname, "max_lfp_voltages.npy"),
            max_lfp_voltages,
            allow_pickle=True,
        )
        for channel in range(num_channels):
            ...
            np.save(
                os.path.join(channel_pathname, "spikes.npy"),
                normed_spikes[channel],
                allow_pickle=True,
            )
            np.save(
                os.path.join(channel_pathname, "spike_times.npy"),
                spike_times[channel],
                allow_pickle=True,
            )
