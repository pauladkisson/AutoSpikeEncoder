...
spike_first_sample = np.load(os.path.join(gt_path, "spike_first_sample.npy"), allow_pickle=True)
...
for channel_folder in os.listdir(session_path):
    ...
    assert len(gt_spike_indices) == len(gt_spike_classes) == len(spike_times) == len(spikes)
